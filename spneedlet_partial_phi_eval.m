function psi_phi = spneedlet_partial_phi_eval( theta, phi, B, j_max )
...
for l = 1:l_max
    for m = 0:l
        coef(l+1, m+bw) = conj(spharmonic_partial_phi_eval(l, m, theta, phi));
    end
end
...
for j = 0:j_max
    ...
    psi_phi{j+1} = inv_spharmonic_tran( alm, Npix, Nring, pre_legendre, tp, pixList, bw, l_st, l_en );
